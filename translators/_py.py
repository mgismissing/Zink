...
class T(Template):
    ...
    def _tuple(s):                                                          return f"({", ".join(s.wt(arg) for arg in s.n[1])}{"," if len(s.n[1]) == 1 else ""})"
    def _list(s):                                                           return f"[{", ".join(s.wt(arg) for arg in s.n[1])}]"
    ...
    def _cast(s):                                                           return f"type({s.wt(s.n[2])}())({s.wt(s.n[1])})"
    ...
    def _func_def(s):               s.indent += 4;                          return f"def {s.n[1]}({", ".join(s.wt(arg) for arg in s.n[2])}) -> {s.wt(s.n[3])}:{"\n".join([""]+([f"{" " * s.indent}super().__init__({", ".join(f"{(s.wt(arg)+":").split(":")[0]}={(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func-assign-super", s.n[2]))})"] if len(list(filter(lambda _: _[0] == "func-assign-super", s.n[2]))) > 0 else [])+[f"{" " * s.indent}self.{(s.wt(arg)+":").split(":")[0]} = {(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func-assign-self", s.n[2])]+s.wt(s.n[4]))}"
    ...
    def _has(s):                                                            return f"({s.wt(s.n[2])} in {s.wt(s.n[1])})"
