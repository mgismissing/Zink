...
class T(Template):
    ...
    def _func_def__(s):                     s.indent += 4;                          return f"def __{s.n[1]}__(self{", " if s.n[2] else ""}{s.jwt(s.n[2], ", ")}):{s.body(([f"{" " * s.indent}super().__init__({", ".join(f"{(s.wt(arg)+":").split(":")[0]}={(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func_assign_super", s.n[2]))})"] if len(list(filter(lambda _: _[0] == "func_assign_super", s.n[2]))) > 0 else [])+[f"{" " * s.indent}self.{(s.wt(arg)+":").split(":")[0]} = {(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func_assign_self", s.n[1])]+s.wt(s.n[3]))}"
    def _func_def__init(s):                 s.indent += 4;                          return f"def __init__(self{", " if s.n[1] else ""}{s.jwt(s.n[1], ", ")}):{s.body(([f"{" " * s.indent}super().__init__({", ".join(f"{(s.wt(arg)+":").split(":")[0]}={(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func_assign_super", s.n[1]))})"] if len(list(filter(lambda _: _[0] == "func_assign_super", s.n[1]))) > 0 else [])+[f"{" " * s.indent}self.{(s.wt(arg)+":").split(":")[0]} = {(s.wt(arg)+":").split(":")[0]}" for arg in filter(lambda _: _[0] == "func_assign_self", s.n[1])]+s.wt(s.n[2]))}"
    def _func_def_async_self(s):            s.indent += 4;                          return f"async {s._func_def_self()}"
    def _func_def_async_untyped(s):         s.indent += 4;                          return f"async {s._func_def_untyped()}"
    def _func_def_async_self_untyped(s):    s.indent += 4;                          return f"async {s._func_def_self_untyped()}"
    def _func_def_async__(s):               s.indent += 4;                          return f"async {s._func_def__()}"
    def _func_def_async__init(s):           s.indent += 4;                          return f"async {s._func_def__init()}"
